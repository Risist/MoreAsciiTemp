...
[CreateAssetMenu(fileName = "ScyheWeapon", menuName = "Character/Weapon/Sctyhe")]
public class CharacterWeaponScythe : CharacterWeaponBase
{
    public override void InitCharacterStateController(CharacterStateController stateController)
    {
        var idle = stateController.AddNewState("scythe_idle");
        ...
        var pushWallDash = stateController.AddNewState("scythe_push wall dash");
        ...
        int cdPush = stateController.CreateCd(0.45f);
        int cdSlash = stateController.CreateCd(0.75f);
        int cdPull = stateController.CreateCd(0.75f);
        ...
        slash
            ...
            .AddComponent(new CState_RotationToDirection(new RangedFloat(0, 0.4f), 0.1f, 0.825f))
            ...
            .AddTransition(pull, new RangedFloat(0.5f, 0.7f), new AnimationTransitionData(0.25f, 0.0f))
        ;
        pull
            .AddComponent(new CState_Input(1))
            .AddComponent(new CState_Cd(cdPull))
            //.AddComponent(new CState_Cd(cdBlock, CState_Cd.EMode.ERestartOnly))
            .AddComponent(new CState_RotationToDirection(new RangedFloat(0, 0.4f), 0.15f, 0.825f))
            .AddComponent(new CState_JumpMotor(new float[] { 230, 155, 200, 200 }, new RangedFloat(0.050f, 0.1250f), -100.0f).SetDefaultDirection(0))
            ...
            .AddTransition(push, new RangedFloat(0.6f), new AnimationTransitionData(0.2f, 0f))
            .AddTransition(slash, new RangedFloat(0.5f, 0.7f), new AnimationTransitionData(0.2f, 0.0f))
        ;

        var jumpPush = new CState_JumpMotor(new float[] { 200, 300 }, new RangedFloat(0.0f, 0.125f))
                .SetDefaultDirection(0);
        push
            .AddComponent(new CState_Input(2))
            .AddComponent(new CState_Cd(cdPush))
            .AddComponent(new CState_RotationToDirection(new RangedFloat(0f, 0.6f), 0.15f, 0.55f))
            .AddComponent(jumpPush)

            .AddComponent(new CState_JumpMotor(new float[] { 275, -180}, new RangedFloat(0.325f, 0.45f))
                .SetDefaultDirection(0))
            ...
            .AddTransition(pushWallDash, new RangedFloat(0.45f, 0.55f), new AnimationTransitionData(0.1f, 0.25f), false, (CharacterState target, CharacterStateController controller) => true)//jumpPush.currentDirectionId == 0)
        ;
        pushWallDash
            .AddComponent(new CState_Input(2))
            .AddComponent(new CStateJumpOverWall(0.65F, 0.04f, 0.25f, 0.75f))
